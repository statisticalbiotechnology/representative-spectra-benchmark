...
class RepresentativeSpectrumCreator:
    ...
    def combine_bin_mean(self, peaklists, minimum=100, maximum=2000, binsize=0.02, apply_peak_quorum=True):
        ...
        merged_spectrum['original_mzs'] = np.empty((len(peaklists), array_size), dtype=np.float32)
        ...
        for peak_i, peaklist in enumerate(peaklists):
            ...
            merged_spectrum['original_mzs'][peak_i][bin_array] = mz_array
        ...
        print(merged_spectrum['original_mzs'])
        print(merged_spectrum['original_mzs'].shape)
        print(merged_spectrum['original'])
        exit(0)
